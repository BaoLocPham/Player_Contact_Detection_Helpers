BASE_DIR = "../input/nfl-player-contact-detection"

TRAIN_LABELS = f"{BASE_DIR}/train_labels.csv"
SAMPLE_SUBMISSION = f"{BASE_DIR}/sample_submission.csv"

TRAIN_TRACKING = f"{BASE_DIR}/train_player_tracking.csv"
TEST_TRACKING = f"{BASE_DIR}/test_player_tracking.csv"

TRAIN_HELMETS = f"{BASE_DIR}/train_baseline_helmets.csv"
TEST_HELMETS = f"{BASE_DIR}/test_baseline_helmets.csv"

TRAIN_META =  f"{BASE_DIR}/train_video_metadata.csv"
TEST_META = f"{BASE_DIR}/test_video_metadata.csv.csv"